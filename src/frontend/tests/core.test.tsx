...
beforeEach(() => { useStore.getState().stop(); useStore.setState({ session: null, history: [], activeRequest: null, lastSeq: 0 }) })
it('uses one shared profile rabbit and accessible button', () => {
  const click = vi.fn()
  render(<Button onClick={click}><RabbitIcon/>새 검색</Button>)
...
describe('layout', () => {
  it('has no card overlap and spreads horizontally', () => {
    ...
    for (const a of sample.nodes) for (const b of sample.nodes) if (a.id !== b.id) {
      expect(Math.abs(a.position.x - b.position.x) >= CARD_WIDTH || Math.abs(a.position.y - b.position.y) >= CARD_HEIGHT).toBe(true)
    }
    const xs = sample.nodes.map(n => n.position.x), ys = sample.nodes.map(n => n.position.y)
  ...
  it('keeps every existing dragged position during expansion', () => {
    const sample = makeSample('vector'), existing = sample.nodes.slice(0, 3)
...
it('ignores stale stream events from a previous search', () => {
  ...
  const event = { version: 1, request_id: 'old', job_id: 'old', seq: 10, type: 'sources', data: { sources: [] } } as Envelope
...
it('rejects nonexistent graph endpoints and deduplicates source events', () => {
  ...
  useStore.setState({ session, activeRequest: 'current', lastSeq: 0 })
  useStore.getState().receive({ version: 1, request_id: 'current', job_id: 'job', seq: 1, type: 'sources', data: { sources: session.sources } })
  expect(useStore.getState().session?.sources).toHaveLength(6)
  useStore.getState().receive({ version: 1, request_id: 'current', job_id: 'job', seq: 2, type: 'relationships', data: { clusters: [], relations: [{ ...session.graph.relations[0], target: 'does_not_exist' }] } })
...
it('preserves cards when relation generation fails', () => {
  ...
  useStore.getState().receive({ version: 1, request_id: 'current', job_id: 'job', seq: 1, type: 'part_error', data: { part: 'relationships', message: 'failed' } })
...
it('never sends sample sources, prices or continuation into real requests', async () => {
  ...
  const fetchMock = vi.spyOn(globalThis, 'fetch').mockResolvedValue(new Response(JSON.stringify({ detail: 'keys missing' }), { status: 503, headers: { 'content-type': 'application/json' } }))
...
it('restores positions and viewport from IndexedDB and deletes history', async () => {
  const sample = makeSample('vector'); sample.viewport = { x: 345, y: -122, zoom: 0.7 }
