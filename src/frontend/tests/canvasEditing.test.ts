...
it('blocks canvas creation, paste and editing on the start page', () => {
  useStore.getState().copyNode('response_entity')
  useStore.getState().newConversation()
  const store = useStore.getState()
  store.createNode({ x: 100, y: 200 })
  store.pasteNode({ x: 100, y: 200 })
  store.editNode('welcome-brand')
  store.editEdge('welcome-example-0')
  store.undo()
  store.redo()
  expect(useStore.getState().session).toBeNull()
  expect(useStore.getState().editingNode).toBeNull()
  expect(useStore.getState().editingEdge).toBeNull()
