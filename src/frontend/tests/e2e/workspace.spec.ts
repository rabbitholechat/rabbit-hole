...
test('compact entity hubs explore copy and send connected information', async ({
  ...
  await expect(hub.locator('.node-tag')).toHaveText('엔티티')
  await expect(hub.locator('.entity-type')).toHaveText('기술')
...
test('sidebar settings show the demo account and shortcuts, preserve the canvas and log out to landing', async ({ page }, testInfo) => {
  await page.goto('/')
  await openHistory(page)
  const viewport = await page.locator('.react-flow__viewport').getAttribute('style')
  const github = page.getByRole('link', { name: 'GitHub (새 탭)', exact: true })
  await expect(github).toHaveAttribute('href', 'https://github.com/rabbitholechat/rabbit-hole')
  await expect(github).toHaveAttribute('target', '_blank')
  const settings = page.getByRole('button', { name: '설정', exact: true })
  await settings.click()
  const dialog = page.getByRole('dialog', { name: '설정', exact: true })
  await expect(dialog).toBeVisible()
  await expect(dialog.getByText('Rabbit', { exact: true })).toBeVisible()
  await expect(dialog.locator('.settings-avatar svg')).toBeVisible()
  await dialog.getByRole('button', { name: '단축키', exact: true }).click()
  await expect(dialog.getByText('선택한 노드 복사', { exact: true })).toBeVisible()
  ...
  expect(geometry!.width).toBeLessThan(page.viewportSize()!.width)
  expect(geometry!.height).toBeLessThan(page.viewportSize()!.height)
  expect(Math.abs(geometry!.x + geometry!.width / 2 - page.viewportSize()!.width / 2)).toBeLessThan(2)
  expect(Math.abs(geometry!.y + geometry!.height / 2 - page.viewportSize()!.height / 2)).toBeLessThan(2)
...
test('welcome canvas examples copy and fill the centered composer without starting a session', async ({ page }, testInfo) => {
  ...
  await expect(page.locator('.welcome-question')).toHaveCount(4)
  await expect(page.getByRole('article', { name: /예시 질문 · 원티드 AI Championship 2026/ })).toHaveCount(1)
  await expect(page.getByRole('article', { name: /예시 질문 · https:\/\/github.com\/rabbitholechat\/rabbit-hole/ })).toHaveCount(1)
...
test('server failures show the branded 500 canvas page and retry without reloading', async ({
  ...
  await expect(errorPage.getByRole('heading')).toHaveText('서버에 오류가 발생했어요')
  await expect(errorPage.getByRole('button', { name: '다시 시도', exact: true })).toHaveCSS('background-color', 'rgb(15, 118, 110)')
...
test('composer tool menu selects per-message tools without changing viewport or uploading files', async ({
  ...
  const viewport = await view.getAttribute('style')
  await add.click()
  ...
  await page.screenshot({ path: testInfo.outputPath('composer-tools.png') })
  await page.keyboard.press('Escape')
  await expect(menu).toHaveCount(0)
  await expect(add).toBeFocused()
  await add.click()
  await menu.getByRole('button', { name: /웹 검색/ }).click()
  await expect(input).toBeFocused()
  await expect(page.getByRole('button', { name: '웹 검색 선택 해제' })).toBeVisible()
  await expect(view).toHaveAttribute('style', viewport!)
  await page.screenshot({ path: testInfo.outputPath('composer-selected.png') })
  await input.fill('웹에서 최신 일정을 찾아줘')
  await input.press('Enter')
  await expect.poll(async () => (await calls(page)).length).toBe(1)
  expect((await calls(page))[0].requested_tool).toBe('web_search')
  await expect(add).toBeDisabled()
  await finishStream(page)
  await expect(add).toBeEnabled()
  await expect(page.getByRole('button', { name: '웹 검색 선택 해제' })).toHaveCount(0)
  await add.click()
  await menu.getByRole('button', { name: /URL 접근/ }).click()
  await input.fill('주소 없이 요약해줘')
  await input.press('Enter')
  await expect(page.getByRole('alert')).toContainText('주소를 넣어')
  expect((await calls(page)).length).toBe(1)
  await input.fill('https://example.com 내용을 정리해줘')
  await page.getByRole('button', { name: '메시지 보내기' }).click()
  await expect.poll(async () => (await calls(page)).length).toBe(2)
  expect((await calls(page))[1].requested_tool).toBe('read_page')
  await finishStream(page)
  await expect(add).toBeEnabled()
  await input.fill('고마워')
  await input.press('Enter')
  await expect.poll(async () => (await calls(page)).length).toBe(3)
  expect((await calls(page))[2].requested_tool).toBeUndefined()
  await finishStream(page)
  await expect(add).toBeEnabled()
  await add.click()
  await input.click()
