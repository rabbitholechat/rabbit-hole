...
export function layoutPages(
  ...
  const links: SimulationLinkDatum<Particle>[] = graph.relations
  ...
  const sim = forceSimulation(particles)
    ...
    .force(
      'y',
      forceY<Particle>((n) => baseY + (sources.findIndex((s) => s.id === n.id) % 2) * 290).strength(0.3),
    )
    .force('collision', forceCollide<Particle>(185).iterations(3))
    .force(
      'links',
      forceLink<Particle, SimulationLinkDatum<Particle>>(links)
        .id((n) => n.id)
        .distance(380)
        .strength(0.08),
    )
