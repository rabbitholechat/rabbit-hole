...
class JobStore:
    ...
    def create(self, client: str) -> Job:
        ...
        self.requests = defaultdict(deque, {k: deque(t for t in v if now - t < 60)
                                           for k, v in self.requests.items() if v and now - v[-1] < 60})
...
def create_app(settings: Settings | None = None, service_factory=AgentService) -> FastAPI:
    ...
    async def search(body: SearchRequest, request: Request):
        ...
        if body.focus_source_id and (not snapshot or body.focus_source_id not in {s.id for s in snapshot.sources}):
        ...
        if not needs_flight and not settings.configured:
            raise HTTPException(503, '검색 API가 설정되지 않았습니다. 백엔드 .env의 키를 설정하세요. 디자인 예시는 별도로 열 수 있습니다.')
        ...
        async def emit(kind: str, data: dict):
            ...
            budget.check()
            seq += 1
            envelope = {'version': 1, 'request_id': str(body.request_id), 'job_id': job.id,
        ...
        async def produce():
            ...
            try:
                ...
                if needs_flight:
                    await emit('clarification', {'kind': 'flight', 'message': '같은 조건으로 비교할 수 있도록 여행 조건을 알려주세요.'})
                ...
                async with asyncio.timeout(settings.job_timeout_seconds):
                    if body.retry_part != 'relationships':
                        ...
                        try:
                            ...
                            await emit('part_error', {'part': 'answer', 'message': '답변을 확인하지 못했습니다. 확보한 페이지는 계속 탐색할 수 있습니다.'})
                    if body.retry_part != 'answer' and registry.sources:
                        ...
                        try:
                            ...
                            await emit('part_error', {'part': 'relationships', 'message': '관계 정리를 완료하지 못했습니다. 페이지 카드는 보존했습니다.'})
            except TimeoutError:
                failed.append(active_part)
                await emit('part_error', {'part': active_part, 'message': '작업 시간이 초과되었습니다. 확보한 결과는 보존했습니다.'})
            except asyncio.CancelledError:
                job.cancel.set()
                raise
            finally:
                try:
                    if not job.cancel.is_set() and not needs_flight:
                        state = Snapshot(query=original_query, sources=list(registry.sources.values()), answer=answer,
                                         relationships=graph, flight=body.flight, issued_at=time.time())
                        await emit('checkpoint', {'continuation': signer.sign(state)})
                        await emit('done', {'status': ('partial' if registry.sources else 'failed') if failed else 'completed',
        ...
        return StreamingResponse(stream(), media_type='text/event-stream', headers={
            'Cache-Control': 'no-cache, no-store', 'X-Accel-Buffering': 'no', 'X-Content-Type-Options': 'nosniff',
        })
