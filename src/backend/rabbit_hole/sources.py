...
async def public_url(raw: str) -> str:
    ...
    records = await asyncio.wait_for(asyncio.get_running_loop().getaddrinfo(host, None, type=socket.SOCK_STREAM), 3)
...
class Registry:
    ...
    def add(self, raw: dict) -> Source:
        ...
        if sid not in self.sources:
            self.sources[sid] = Source(
                id=sid, original_url=original, url=url, domain=urlsplit(url).hostname or '',
                title=str(raw.get('title') or url)[:400], summary=str(raw.get('content') or '')[:2500],
                # Search does not guarantee publication dates. Never infer them.
                published_at=raw.get('published_date'), retrieved_at=utc_now(),
            )
    ...
    def validate_relationships(self, graph: Relationships) -> Relationships:
        ...
        for edge in graph.relations:
            if edge.source == edge.target or edge.source not in self.sources or edge.target not in self.sources:
