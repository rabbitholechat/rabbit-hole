...
- `web_search(query, temporal_focus="unspecified")`: 최대 2000자. `current/historical/unspecified` 시간 의도를 일반 에이전트가 선택하며 주제별 분기 없음. 질의는 그대로 보존하고 서울 기준 `reference_date`, 초 단위 `reference_time`을 별도로 전달. `current`에는 현재 시각까지 최근 5일의 `search_window.since/until`, 우선 탐색할 최근 3일의 `prefer_since`를 전달하며 실제 검색에 연·월·일 범위를 반영하도록 지시. 명시적인 사용자 기간이 우선하며 다른 시간 의도에는 window=null. 날짜 범위는 검색 모델의 탐색 지침이고 검색 결과를 서버에서 날짜로 필터링하거나 최신성을 보증하는 기능은 아님. 발행/업데이트일과 사건일을 구분하고, 미래 시각·크롤링 시각을 최근 발표로 취급하지 않음. 기간 내 확인되지 않으면 한계를 표시하고 이전 발표는 날짜를 명시한 배경으로만 구분. 현재 연도를 무조건 질의에 붙여 기존 최신 발표나 연도 없는 공식 페이지를 누락시키지 않음. `external_web_access: true`와 검색 컨텍스트 medium 유지. 검색 1회당 내장 도구 1회·출력 1,500토큰, 전체 예산·취소 유지. 같은 요청을 기계적으로 재검색하지 않으며 필요할 때만 남은 예산으로 질의 수정 또는 본문 확인.
