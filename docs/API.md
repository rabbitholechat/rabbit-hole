...
- `web_search(query, temporal_focus="unspecified")`: 최대 2000자. `current/historical/unspecified` 시간 의도를 일반 에이전트가 선택하며 주제별 분기 없음. `current`는 서버 서울 날짜의 현재 연도를 질의에 추가(이미 있으면 중복 생략), 역사 질의는 변경하지 않음. 검색 모델 입력과 내부 결과에 `reference_date/temporal_focus` 포함. 당일만으로 제한하는 날짜 필터는 사용하지 않음. `external_web_access: true` 명시(공식 API 기본값도 true이며 캐시 전용 상태였다는 뜻은 아님). 오래된 결과뿐이면 남은 검색에서 현재 범위로 재질의하고 최신 여부 미확인 상태로 표시. `OPENAI_SEARCH_MODEL`로 OpenAI Responses 내장 `web_search`를 한 번 호출하며 검색 요약과 실제 응답 메타데이터의 URL만 반환. 요약은 모델 생성 요약이며 본문 인용문이 아님. 출처 메타데이터가 없으면 요약도 사용하지 않음. 검색 모델에는 원래 사용자 요청을 함께 전달하여 고유명사와 대상을 보존. 도구 결과는 남은 검색·전체 도구 예산과 후보 URL의 관련성/사실성이 미검증임을 알리는 usage_notice 포함(내부 도구 계약, 공개 SSE에 추가하지 않음). 무관한 인물·주제는 답변 근거로 사용하지 않고 남은 예산에서 정확한 이름·주제 또는 확인된 별칭으로 재질의. 의미 관련성 판단은 모델이 수행하며 서버에서 URL 존재만으로 관련성을 보증하지 않음.
