...
기존 `/api/structure`의 도구 없는 단일 호출에서 정보 카드와 최대 4개의 탐색 대상 Entity를 함께 추출합니다. 별도 검색·Entity 추출·관계 생성 호출은 추가하지 않습니다. 기존 v1/v2 결과와 기록을 계속 읽으며, Entity 후보를 포함한 결과는 version=3입니다. Entity가 없는 결과는 version=2일 수 있습니다.

`entities`는 `{key, name, subtype, qualifier, aliases, role, references, links}` 배열입니다. subtype은 concept/technology/company/product/person, role은 해당 응답에서 main/related입니다. qualifier는 원문에 명시된 분야·제조사 등 구분 문구 또는 null입니다. 같은 단일 구조화 호출에서 Entity를 먼저 식별·출력한 뒤 정보 카드를 구성하도록 지시합니다. 이름·별칭·구분 문구의 `references: TextSpan[]`는 답변 제목 등 카드 밖의 원문도 참조할 수 있습니다. links는 `{item_key, references: TextSpan[]}`이며 정보 카드의 원문 참조 범위 안에서 그 대상을 설명하는 부분을 지정합니다. 연결마다 대상 이름이 반복될 필요는 없지만 문맥상 대상 관계가 있어야 합니다. 잘못된 연결만 제외하고 유효한 연결을 보존하며, 이름 근거가 잘못되거나 유효 연결이 없는 Entity는 제외합니다. Entity 정체성 근거는 응답별 observations.references에 저장합니다. 이전 v3에서 references가 누락되면 유효 연결의 원문 참조를 사용하며 기존 저장 기록을 재추출하지 않습니다. 이는 원문 범위 검증이며 의미적 관계나 외부 사실의 검증을 보증하지 않습니다. 모델의 형식 오류·출력 한도 실패는 원래 답변에서 격리합니다.

프런트 `contentGraph.entities`에 type=entity를 추가합니다. 새 결과는 응답 → Entity(kind=has_entity, 대상) → 정보(kind=has_information, 관련 정보)로 연결합니다. 정보의 원래 응답 참조인 has_extract도 저장하지만 Entity를 통해 연결된 카드에는 직통선을 표시하지 않습니다. 여러 대상을 비교하는 정보는 여러 Entity가 공유하며 복제하지 않습니다. Entity가 없는 유효한 정보는 응답에 직접 연결합니다. 이전 about(정보 → Entity) 기록은 기존 방향과 좌표로 복원합니다. Entity 간 의미 관계, supported_by, represented_by는 생성하지 않습니다. Main 여부는 응답별 observations에 보존하며 공유 허브에는 중심으로 다룬 응답이 존재하는지만 표시합니다.

같은 응답의 정보 카드들은 Entity를 공유합니다. 후속 응답에서는 같은 subtype, 공백·대소문자를 정리한 이름 또는 원문 별칭, 동일한 비어 있지 않은 qualifier가 모두 일치할 때만 같은 세션의 기존 허브를 재사용합니다. 불분명한 동명 대상과 서로 다른 세션은 통합하지 않습니다. 원문 범위 검증만으로 별칭의 의미상 동일성을 완전히 보증하지는 않습니다.
