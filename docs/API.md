...
web_search가 실제 출처를 반환하면 같은 검색어로 Wikimedia Commons 이미지를 자동 검색합니다. 별도 이미지 요청이 없어도 적용되며, 실패/빈 웹 검색에는 실행하지 않습니다. MAX_IMAGE_SEARCHES는 요청당 이미지 검색 한도(기본 1, 범위 0..5)이며 명시적 이미지 검색과 공유합니다. MAX_TOOL_CALLS도 공유하고 기존 MAX_WEB_SEARCHES는 웹 검색만 계산합니다. 이미지 검색 실패는 웹 결과에 영향을 주지 않습니다.

서버 출처 상한 안에서 일반 페이지를 먼저 유지하고 관련 이미지 최대 2개 자리를 확보합니다. 상한이 1이면 웹 출처를 우선합니다. 이미지만 요청한 경우 기존 출처 상한까지 이미지로 표시합니다. 이미지 카드는 보라색 배경·테두리·선택 강조 및 ‘이미지’ 태그로 구분하며 미리보기·제목·원본 페이지 링크를 표시합니다. 검색 결과가 없으면 관련 없는 대체 이미지를 만들지 않습니다.

### 이미지 검색 빈 결과 보완

Commons 검색 결과가 없거나 이미지 검색 호출이 실패했을 때, 실제 웹 검색이 반환한 상위 페이지 최대 2개에서 og:image/twitter:image 메타데이터를 읽습니다. 한국어·긴 최신 정보 검색어에서 Commons 결과가 없더라도 원본 페이지가 제공한 대표 이미지를 사용할 수 있습니다. 별도 생성 URL이나 무관한 샘플 이미지로 대체하지 않습니다.

페이지 조회는 기존 도구 호출 예산·동시 수·안전 URL/DNS/TLS/용량 제한을 적용합니다. 이미지 URL도 HTTPS 및 공개 DNS를 검사하고 동일 페이지 ID/제목/원본 링크를 유지합니다. 이 경로의 썸네일 호스트는 Wikimedia에 한정되지 않으며 실제 페이지 메타데이터의 검증된 공개 이미지 URL을 사용합니다. 페이지당 한 출처 노드를 유지하고 대표 이미지가 확보된 노드는 보라색 이미지 카드로 표시합니다. 추가 페이지 조회도 실패하면 이미지 카드를 만들지 않습니다. 저장된 과거 기록에는 자동 소급 조회하지 않습니다.
