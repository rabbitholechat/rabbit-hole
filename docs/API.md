...
- `web_search(query, temporal_focus="unspecified")`: 최대 2000자. `current/historical/unspecified` 시간 의도를 일반 에이전트가 선택하며 주제별 분기 없음. `current`는 서버 서울 날짜의 현재 연도를 질의에 추가(이미 있으면 중복 생략), 역사 질의는 변경하지 않음. 검색 모델 입력과 내부 결과에 `reference_date/temporal_focus` 포함. 일반 최신 질문은 정확한 대상·주제와 현재 연도를, 오늘·방금 등 당일 민감 질문은 전체 `reference_date` 또는 명시적 최근 범위를 질의에 반영. 검색 순위를 최신성 근거로 취급하지 않고, 게시/업데이트 날짜와 실제 사건 날짜를 비교. 당일만으로 제한하는 날짜 필터는 사용하지 않음. `external_web_access: true` 명시(공식 API 기본값도 true이며 캐시 전용 상태였다는 뜻은 아님). 오래된 결과뿐이면 남은 검색에서 공식 발표/제품·상태 페이지 또는 최근 보도 등 서로 다른 현재 범위로 재질의하고 최신 여부 미확인 상태로 표시. 첫 결과가 요청 사실을 충분히 확립하면 두 번째 검색을 일괄 실행하지 않음. `OPENAI_SEARCH_MODEL`로 OpenAI Responses 내장 `web_search`를 한 번 호출하며 검색 요약과 그 요약이 실제로 인용한 `url_citation` URL만 반환. 원시 검색 후보 `action.sources`는 답변과 무관할 수 있으므로 출처 등록·노드 생성에서 제외. 요약은 모델 생성 요약이며 본문 인용문이 아님. 인용 출처 메타데이터가 없으면 요약도 사용하지 않음. 검색 모델에는 원래 사용자 요청을 함께 전달하여 고유명사와 대상을 보존. 도구 결과는 남은 검색·전체 도구 예산과 인용 URL의 관련성/사실성이 미검증임을 알리는 usage_notice 포함(내부 도구 계약, 공개 SSE에 추가하지 않음). 무관한 인물·주제는 답변 근거로 사용하지 않고 노드로도 생성하지 않으며, 남은 예산에서 정확한 이름·주제 또는 확인된 별칭으로 재질의. 의미 관련성 판단은 모델이 수행하며 서버에서 URL 존재만으로 관련성을 보증하지 않음.
...
`entities`는 `{key, name, subtype, qualifier, aliases, role, links}` 배열입니다. subtype은 concept/technology/company/product/person, role은 해당 응답에서 main/related입니다. qualifier는 원문에 명시된 분야·제조사 등 구분 문구 또는 null입니다. links는 `{item_key, references: TextSpan[]}`이며 해당 정보 카드의 원문 참조 범위 안에 있고 Entity 이름을 포함해야 합니다. 이름·별칭·구분 문구를 원문에서 확인할 수 없거나 연결이 잘못된 Entity는 제외하고 유효한 정보 카드는 유지합니다. 이는 답변의 원문 참조이며 외부 사실 검증이 아닙니다. 모델의 형식 오류·출력 한도 실패는 기존 구조화 실패 처리와 동일하게 원래 답변에서 격리합니다.

프런트 `contentGraph.entities`에 type=entity를 추가합니다. 정보 → Entity는 kind=about(설명 대상), 기존 has_extract/cites/consulted/related_image와 구별합니다. Entity 간 의미 관계, supported_by, represented_by는 생성하지 않습니다. Main 여부는 응답별 observations에 보존하며 공유 허브에는 중심으로 다룬 응답이 존재하는지만 표시합니다.
...
캔버스 type=entity는 코랄색 소형 카드입니다. 정보 다음 열에 생성하며 출처와 이미지 열은 더 오른쪽에 배치합니다. 이미 생성된 노드의 위치와 viewport는 유지합니다. 클릭 시 연결된 정보를 강조하고 관련 정보 목록에서 기존 노드로 이동합니다. 탐색·저장 기록 복원은 모델/검색 호출 없이 이루어집니다. Entity와 연결·등장 기록·접힘 상태는 기존 PostgreSQL 세션 스냅샷에 저장합니다. 이전 기록을 재추출하거나 Entity를 소급 생성하지 않습니다.
