...
기본 카드는 `엔티티 · 제품`처럼 subtype을 엔티티 바로 옆에 표시하고 이름·관련 정보 수를 보여줍니다. 복사/펼치기/다음 응답 사용 아이콘은 다른 콘텐츠 카드와 같은 동작입니다. 펼치면 구분 문구·별칭·관련 정보 제목 목록을 표시합니다. 복사는 엔티티 이름·유형·별칭·구분 문맥과 직접 연결된 정보 카드의 내용을 포함하며, 같은 정보에 대한 중복 관계는 한 번만 포함합니다. 이전 about 관계도 지원하며 무관한 정보·응답·출처 내용을 자동으로 넣지 않습니다.
